...
const createContact = (request, response) => {                                //  CRUD1
  const { id2, email, mobile, xstatus } = request.body
  pool.query('INSERT INTO XCONTACTS (id2,email,mobile,xstatus) VALUES ($1,$2,$3,$4)',
...
const getContacts = (request, response) => {                                 // GET 2
...
const getContact = (request, response) => {                                  // GET 2
  const id = parseInt(request.params.id2)
  pool.query('SELECT * FROM XCONTACTS WHERE id2 = $1', [id], (error, results) => {
    if (error) { throw error }
    response.status(200).json({
      status: 'success',
      data: results.rows,
...
const updateContact = (request, response) => {                                //  UPDATE 
  const id = parseInt(request.params.id2)
  const { email, mobile, xstatus } = request.body
  pool.query('UPDATE XCONTACTS SET email=$1, mobile=$2, xstatus=$3 WHERE id2 = $4',
